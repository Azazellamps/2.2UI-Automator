...
//    }
    ...
    fun testEmptyChangeText() {
        ...
        val textWelcome = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(result, textWelcome)
    }
    @Test
    fun testSpaceChangeText() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        val result = device.findObject(By.res(packageName, "textToBeChanged")).text


        device.findObject(By.res(packageName, "userInput")).text = textSpace
        device.findObject(By.res(packageName, "buttonChange")).click()
